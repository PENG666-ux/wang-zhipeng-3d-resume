...
const COPY = {
  en: {
    eyebrow: 'Target · Part-time Motion Sensor Algorithm Validation Engineer',
    title: 'Zhipeng Wang',
    paragraphs: [
      'M.Sc. candidate in Civil Engineering at Sun Yat-sen University, focusing on structural dynamics, ultrasonic guided waves, sensor signal analysis, and structural damage identification.',
      'I combine mechanics, sensing, and AI algorithms to model physical mechanisms, validate data-driven models, and automate repeatable engineering workflows.',
    ],
  },
  zh: {
    eyebrow: '目标岗位 · 兼职运动传感器算法验证工程师',
    title: '王智鹏',
    paragraphs: [
      '中山大学土木工程硕士研究生，研究结构动力响应、超声导波、传感信号分析处理与结构损伤识别。',
      '把力学机理、运动与振动传感、AI 算法连接起来，开展模型构建、训练调参、验证评估与工程流程自动化。',
    ],
  },
}

function Hero({ lang, cueOpacity }: { lang: Lang; cueOpacity: MotionValue<number> }) {
  const { eyebrow, title, paragraphs } = COPY[lang]
    ...
    <section className="hero">
      ...
      >
        ...
        <div className="about-intro">
          <motion.div className="about-eyebrow" style={{ y: bodyY }}>
            {eyebrow}
          </motion.div>
    ...
    <>
      ...
      <motion.div className="hero-chrome" style={{ opacity: heroChromeOpacity }} aria-hidden="true">
        ...
        <div className="hero-meta hm-tl">
          <span className="hm-name">Zhipeng Wang 王智鹏</span>
          <span>Motion Sensing · Structural Dynamics · Physics-informed AI</span>
        </div>
        <img
          className="hero-profile"
          src={`${import.meta.env.BASE_URL}images/wang-zhipeng-profile.png`}
          alt=""
        />
        <div className="hero-meta hm-tr">3D Résumé — 2026</div>
        <div className="hero-meta hm-bl">183 0880 5827 · wangzhp29@mail2.sysu.edu.cn</div>
        <div className="hero-meta hm-right">PZT × 81 · Rail-PINN · Ski</div>
      </motion.div>
